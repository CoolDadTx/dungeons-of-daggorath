...
namespace DoD.CommandLine
{
   ...
   //         byte c = 0;
	    ...
	    //C++ TO C# CONVERTER TODO TASK: Pointer arithmetic is detected on the parameter 'byte* X', so pointers on this parameter are left unchanged:
	    //C++ TO C# CONVERTER TODO TASK: Pointer arithmetic is detected on the parameter 'int* Xup', so pointers on this parameter are left unchanged:
	    //C++ TO C# CONVERTER TODO TASK: Pointer arithmetic is detected on the parameter 'byte* U', so pointers on this parameter are left unchanged:
  ...
  //          int U = 0;
        ...
        //HACK: Renamed 'Parser' to allow for compilation
		public int ParserDoSomething (byte[] pTABLE, ref byte A, ref byte B, bool norm)
		{
            throw new NotImplementedException();
  //          bool tok;
		//	int U;
		...
		//	if (B != STRING[Y++])
		//		goto PARS30;
		//	if (STRING[Y] != (int)AnonymousEnum.INull && STRING[Y] != 0)
		//		goto PARS12;
   ...
   //         LINPTR = 0;
		...
		public ushort LINPTR {get; set;}
		public byte PARFLG {get; set;}
		public byte PARCNT {get; set;}
		public byte VERIFY {get; set;}
		public byte FULFLG {get; set;}
		public byte KBDHDR {get; set;}
		public byte KBDTAL {get; set;}
		public byte BUFFLG {get; set;}
		public byte[] KBDBUF {get; set;} = new byte[33];
		public byte[] LINBUF {get; set;} = new byte[33];
		public ushort LINEND {get; set;}
		public byte[] TOKEN {get; set;} = new byte[33];
		public byte TOKEND {get; set;}
		public byte[] STRING {get; set;} = new byte[35];
		public byte[] SWCHAR {get; set;} = new byte[11];
		public byte[] OBJSTR {get; set;} = new byte[33];
		public byte[] CMDTAB {get; set;} = new byte[69];
		public byte[] DIRTAB {get; set;} = new byte[26];
        public byte[] M_PROM1 { get; set; } = new byte[5];
        public byte[] M_CURS { get; set; } = new byte[3];
        public byte[] M_ERAS { get; set; } = new byte[6];
        public byte[] CERR { get; set; } = new byte[3];
